...
list_finding_dentist = ["Missing", "Implant", "Remnant Root", "Caries", "Root Filled", "Crown Bridge", "Apical Lesion", "Restorations"]
...
# raw data -> golden.csv
def Golden(
        dataset_name: str = "ntuh"
) -> pd.DataFrame:
    if dataset_name == "ntuh":
        sheet = pd.read_csv("/mnt/hdd/PANO.arlen/data/raw_data/(WIP) NTUH Summary Golden Label - Per-study.csv")
    ...
    # df_result.to_csv("/mnt/hdd/PANO.arlen/golden/ntuh_golden.csv", index=False)
...
def dentist_review(
    ...
    pano_finding_1_df = pd.read_csv("/mnt/hdd/PANO.arlen/data/raw_data/Pano Finding - 1 (Responses) - Form Responses 1.csv")
    pano_finding_2_df = pd.read_csv("/mnt/hdd/PANO.arlen/data/raw_data/Pano Finding - 2 (Responses) - Form responses 1.csv")
    pano_finding_3_df = pd.read_csv("/mnt/hdd/PANO.arlen/data/raw_data/Pano Finding - 3 (Responses) - Form responses 1.csv")
    pano_finding_4_df = pd.read_csv("/mnt/hdd/PANO.arlen/data/raw_data/Pano Finding - 4 (Responses) - Form responses 1.csv")
    ...
    pano_finding = [[pano_finding_1_df, pano_image_1_df], [pano_finding_2_df, pano_image_2_df], [pano_finding_3_df, pano_image_3_df], [pano_finding_4_df, pano_image_4_df]]
    dentist_list = []
    # total 120 images
    image_list = pd.concat([image[1] for df, image in pano_finding], ignore_index=True).values.tolist()
                    ...
                    dentist_list.append(
                        {
                            ...
                            "finding": dict_finding_dentist[filtered_result[i].values[0]]
                        }
                    )
    ...
    for finding_name in list_finding_PANO:
        ...
        # no missing tooth
            ...
            for i in list_dentist:
                ...
                df_full_tooth = df_full_tooth[df_full_tooth["missing"] == 0].drop(columns=['missing'])
...
for dentist in dentist_name:
    ...
    dentist_dataframe.to_csv("tmp/dentist_{}.csv".format(dentist), index=False)
...
for id, finding_name in enumerate(list_finding_PANO):
    ...
    plt.plot(fpr[id], tpr[id], color='darkorange', lw=2, label='ROC curve (AUC = {:.2f})'.format(roc_auc[id]))
